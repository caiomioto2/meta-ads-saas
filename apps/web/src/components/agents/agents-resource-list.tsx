...
import { useAgents, useRemoveAgent, useAuditEvents } from "@decocms/sdk";
...
export function AgentsResourceList({
  ...
  const handleAgentClick = (item: Record<string, unknown>) => {
    const agent =
      (item._agent as import("@decocms/sdk").Agent) ||
      (item as unknown as import("@decocms/sdk").Agent);
    focusChat(agent.id, crypto.randomUUID(), { history: false });
  };

  const handleDuplicate = async (agent: import("@decocms/sdk").Agent) => {
    try {
      const newAgent = {
  ...
  const handleOpen = (agent: import("@decocms/sdk").Agent, uri: string) => {
    addTab({
      type: "detail",
      resourceUri: uri,
